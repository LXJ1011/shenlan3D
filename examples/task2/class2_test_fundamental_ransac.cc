...
int  calc_ransac_iterations (double p,
                           int K,
                           double z = 0.99){
    double all_inliers = math::fastpow(p, K);
    double num_iterations = std::log(1.0 - z)/std::log(1.0 - all_inliers);
    /* 计算迭代次数*/
...
std::vector<int> find_inliers(sfm::Correspondences2D2D const & matches
    ...
    //思路：遍历所有匹配点，计算sampson Distance,与给定阈值作比较。
    for(std::size_t i = 0; i < matches.size(); i++)
    {
        //sfm::Correspondence2D2D const& p = matches[i];
...
int main(int argc, char *argv[]){
    ...
    //std::ifstream in("./examples/task2/correspondences.txt");//原始错误相对路径！
    std::ifstream in("../../../examples/task2/correspondences.txt");//原来真是文件路径的问题！
    //std::ifstream in("/home/lxj/下载/code/ImageBasedModellingEdu/ImageBasedModellingEdu/examples/task2/correspondences.txt");
    assert(in.is_open());
